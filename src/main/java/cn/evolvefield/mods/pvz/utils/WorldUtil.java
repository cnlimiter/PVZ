...
public class WorldUtil {
	...
	/**
	 * {@link ZombieHandEntity#spawnRangeZombieHands(Level, PVZZombieEntity, int)}
	 */
	 ...
	 * {@link EntityUtil#spawnStaticParticle(Entity, ParticleOptions)}
	 */
	...
	public static int calculateGenHeight(LevelReader worldIn, int x, int z){
	...
	public static BlockPos findRandomSpawnPos(Level world, BlockPos center, int chance, int minRange, int maxRange, Predicate<BlockPos> predicate) {
		...
		for (int i = 0; i < chance; ++i) {
			...
			if (world.hasChunksAt(pos.offset(-range, -range, -range), pos.offset(range, range, range))
					&& world.getChunkSource().hasChunk(pos.getX(), pos.getZ())) {
